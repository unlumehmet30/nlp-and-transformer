...
class TransformerModel(tf.keras.Model):
    def __init__(self, num_layers, embed_size, heads, input_dim, output_dim, dropout_rate):
        ...
        self.fc = layers.Dense(output_dim, activation="sigmoid")  # binary classification için sigmoid

    def call(self, x, training=None):
        x = self.embedding(x)
        for transformer in self.transformer_blocks:
            x = transformer(x, training=training)  # 💡 Burada düzeltildi!
...
dummy_input = tf.random.uniform((1, 100), minval=0, maxval=10000, dtype=tf.int32)
model(dummy_input)
